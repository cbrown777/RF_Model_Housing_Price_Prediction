...
```{r}
rm(list=setdiff(ls(), "geocoded")) #Import Data and Load Packages
set.seed(1001)
...
geocoded <- setNames(data.frame(matrix(ncol = 2, nrow = 2230)), c("lon", "lat"))
View(geocoded)
...
#*****
for(i in 1:nrow(q_housing)){
  result <- geocode(q_housing$full_address_or_zip_code[i], output = "latlona", source = "google")
...
which(is.na(q_housing$lon))
q_housing = q_housing[-c(804, 1679), ]
summary(q_housing$lat)
which(is.na(q_housing$lat))
...
#****
rm(fit, M, m1, plot1, X, Ximp, Xnew, y)
rm(q_housing1)
#****
...
#Filter out apts that dont have sale price
q_housing1 %<>%
...
ncol(Xnew)
lapply(Xnew, class) #See class type for all columns
...
head(model.matrix(sale_price ~ ., Xnew))
...
pacman::p_load(missForest)
Ximp2 = missForest(data.frame(test_set), sampsize = rep(25, ncol(test_set)))$ximp
...
#OOB R-sqrd and RMSE
my_forest
#OOB RMSE is $75858.86 and R^2 is 0.8213
...
#Hold-out RMSE is 84970.92 and R^2 is 0.7747
...
#train a model
rforest <- train(rf.tree, regr_task)
```
